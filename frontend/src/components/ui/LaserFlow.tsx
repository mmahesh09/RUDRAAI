...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const setSizeNow = () => {
      ...
      if (
        Math.abs(w - last.width) <= 0.5 &&
        Math.abs(h - last.height) <= 0.5 &&
        Math.abs(pr - last.dpr) <= 0.01
      ) return;
    ...
    const updateMouse = (clientX: number, clientY: number) => {
      ...
      mouseTarget.set((clientX - rect.left) * ratio, hb - (clientY - rect.top) * ratio);
    };
    ...
    const adjustDpr = (now: number) => {
      ...
      if (avgFps < lowerThresh) next = clamp(currentDprRef.current * 0.85, dprFloor, base);
      else if (avgFps > upperThresh && currentDprRef.current < base) next = clamp(currentDprRef.current * 1.1, dprFloor, base);
